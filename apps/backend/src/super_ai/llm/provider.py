...
class NoopRerankModel:
    """Pass-through reranker: returns documents in original order with score 1.0.

    Used by providers (e.g. DeepSeek) that don't offer a native rerank API.
    The retrieval pipeline will still fuse results with RRF, just without
    a separate semantic rerank pass.
    """
    ...
    async def arerank(
        ...
        *,
        ...
        return [
            RerankResult(index=idx, relevance_score=1.0)
            for idx in range(min(top_n, len(documents)))
        ]


def build_default_llm_provider(config_path: Path | str | None = None) -> LlmProvider:
    """Build the default configured LLM provider (backward-compatible)."""
    return build_llm_provider(config_path=config_path)


def build_llm_provider(config_path: Path | str | None = None) -> LlmProvider:
    """Build a configured LLM provider, routing by config.provider field.

    Supported providers:
      - qwen-openai      → Qwen, OpenAI-compatible (default, with rerank)
      - deepseek-openai  → DeepSeek via OpenAI-compatible API (no rerank)
    """
    config = load_llm_provider_config(config_path=config_path)
    provider_name = config.provider

    if provider_name == "deepseek-openai":
        return QwenOpenAIProvider(
            config,
            rerank_factory=_create_noop_rerank_model,
        )

    # Default: qwen-openai or any unknown provider
...
def _create_noop_rerank_model(config: LlmProviderConfig) -> RerankModel:
    """Return documents in original order — for providers without rerank (e.g. DeepSeek)."""
