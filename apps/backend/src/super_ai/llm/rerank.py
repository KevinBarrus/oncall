...
class SiliconFlowRerankModel:
    """SiliconFlow BGE / Qwen3 Reranker HTTP client (OpenAI-compatible /rerank endpoint).

    Request format: ``{"model": "...", "query": "text", "documents": [...], "top_n": N}``
    Response format: ``{"results": [{"index": 0, "relevance_score": 0.85}]}``
    """
    ...
    async def arerank(
        ...
        *,
        ...
        return _parse_siliconflow_results(response, document_count=len(documents), top_n=top_n)
...
def _parse_siliconflow_results(
    ...
    """Parse SiliconFlow / Cohere-style flat response format."""
