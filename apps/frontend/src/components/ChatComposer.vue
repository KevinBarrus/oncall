  ...
  <form class="chat-composer" @submit.prevent="submit">
    ...
    <div class="chat-composer__row">
      <div class="chat-composer__field">
        <textarea id="chat-message" ref="textareaRef" v-model="content" rows="1" placeholder="向 Super AI 描述你正在处理的问题" :disabled="inputDisabled" @keydown="handleKeydown" />
...
textarea { background: transparent; border: 0; color: var(--text-primary); line-height: 1.55; max-height: 10rem; min-height: 2.8rem; outline: 0; overflow-y: auto; padding: 0.62rem 0.55rem 0.5rem 0.75rem; resize: none; width: 100%; }
textarea::placeholder { color: var(--text-tertiary); }
.chat-composer__field button { align-self: end; align-items: center; background: var(--text-primary); border-radius: 50%; color: var(--btn-send-text, #fff); display: inline-flex; height: 2.35rem; justify-content: center; margin: 0 0.15rem 0.15rem 0; transition: background var(--transition-fast), transform var(--transition-fast); width: 2.35rem; }
.chat-composer__field button:hover:not(:disabled) { background: var(--accent-strong); transform: translateY(-1px); }
.chat-composer__field button:disabled { background: var(--btn-send-disabled, #d7d7da); color: var(--btn-send-text, #fff); }
