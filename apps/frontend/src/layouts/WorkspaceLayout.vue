...
import { CircleDot, LogOut, Moon, Sparkles, Sun, TriangleAlert } from "lucide-vue-next";
  ...
  <div class="workspace-layout">
    <aside class="workspace-layout__rail">
      ...
      <div class="workspace-layout__account">
        ...
        <button type="button" :title="theme.isDark ? '切换亮色模式' : '切换暗色模式'" :aria-label="theme.isDark ? '切换亮色模式' : '切换暗色模式'" @click="theme.toggle()">
...
.workspace-layout__brand svg { color: var(--brand-icon); grid-row: span 2; }
...
.workspace-layout__account { align-items: center; border-top: 1px solid var(--rail-divider); display: grid; gap: 0.65rem; grid-template-columns: auto minmax(0, 1fr) auto auto; margin: auto 0.3rem 0; padding: 1rem 0.25rem 0.1rem; }
.workspace-layout__avatar { align-items: center; background: var(--avatar-bg); border-radius: 50%; color: var(--avatar-text); display: inline-flex; font-size: 0.78rem; font-weight: 700; height: 2rem; justify-content: center; width: 2rem; }
...
.workspace-layout__account button:hover { background: var(--rail-hover); color: var(--workspace-icon-active); }
.workspace-layout__main { display: grid; grid-template-rows: auto minmax(0, 1fr); height: 100dvh; min-width: 0; overflow: hidden; }
.workspace-layout__header { align-items: center; background: var(--header-glass); border-bottom: 1px solid var(--line); display: flex; justify-content: space-between; min-height: 4.65rem; padding: 0.85rem clamp(1.25rem, 3vw, 3.25rem); position: sticky; top: 0; z-index: 4; }
