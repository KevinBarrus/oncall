...
- 已完成：`_determine_contract` 注释与 Replanner 决策消息对齐为“规则驱动的受限重规划/回退”，明确不调用 LLM 重新规划。
- 已完成：README 与 OpenSpec 主规格同步表述（Replanner 依据确定性规则决定继续/知识库回退/报告）。
- 限制：`autonomous_replan` 契约名保留（公开 SSE 契约字段），语义澄清为“规则触发的知识库回退”，非 LLM 自主重规划；不引入 LLM 重规划。
