...
- 已完成：新增租户 filter 互斥测试（tenant_a 的 filter 不含 user_b 值、反之亦然）与 filter 转义测试（作用域值含引号时安全转义，防注入）。
- 复核结论：全部 Milvus 数据访问操作均要求显式 tenant 作用域，无 `count_chunks` 等遗漏操作；确认项关闭。
...
- 已完成：`sseClient` 连接建立阶段（fetch + 2xx 检查）有限重试 2 次（500ms/1s 退避）；非 2xx 服务端错误不重试；重试每次重新取 token。
- 已补充测试：连接失败重试成功、服务端错误不重试、重试耗尽抛错。
- 限制（design 记录）：流开始后的中断不自动重发 POST（会重复执行 Agent 与重复持久化消息；chat store 已有“回答流意外中断”检测、aiops 诊断由后台任务 + 证据链重载兜底）；断点续传（`from_sequence`）需后端事件缓存与重放，成本高未实现。
